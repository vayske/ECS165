...
class Bufferpool:
    ...
    def __init__(self):
        self.empty = [i for i in range(BUFFERPOOL_SIZE)]
        self.used = deque()
        self.pinned = np.zeros(BUFFERPOOL_SIZE, dtype=int)
        self.LRUIndex = np.empty(BUFFERPOOL_SIZE, dtype=int)
        self.pool = [Page("", ()) for i in range(BUFFERPOOL_SIZE)]  # empty page, will be replace by real page
