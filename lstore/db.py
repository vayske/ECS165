...
class Bufferpool:
    ...
    def __init__(self):
        ...
        self.directory = defaultdict(lambda: -1)  # (table, b or t, page_index, page_num, column_number) -> index in pool
    ...
    def flush_pool(self):
        for page in self.pool:
            if page.dirty:
                ...
                data_str = page.data
                file.write(num_records_to_bytes)
                file.write(data_str)
                file.close()
    ...
    def getindex(self, table, bt, page_index, page_num, column):
        i = self.directory[(table, bt, page_index, page_num, column)]
        if i == -1:
            if len(self.used) == len(self.pool):
                self.evict()
            path = os.getcwd() + "/" + table + "/" + bt + "_" + str(page_index) + "_" + str(page_num) + "c_" + str(column)
            i = self.get_from_disk(path, (table, bt, page_index, page_num, column))
            self.directory.update({(table, bt, page_index, page_num, column): i})
        self.used.remove(i)
        self.used.append(i)
        return i
