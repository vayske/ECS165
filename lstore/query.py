...
class Query:
    ...
    #---------- Create New pages ------------------#     
                ...
                self.table.tail_directory.update({str(new_page_index): 0})
    ...
    def update(self, key, *columns):      
        ...
        if(len(ridList) == 0):
            ...
            return None
        page_index, slot = self.table.page_directory[str(rid)]
         # get the latest tail page number for this base page
        latest_tail_number = self.table.tail_directory[str(page_index)]
# ------ Read Schema Code for Checking Updated Data ------ #
        ...
        latest_tail_page_index = self.table.bufferpool.getindex(self.table.name, "t", page_index, latest_tail_number, self.table.key)
        
        #create new tail page if lastest tail page is full
        if self.table.bufferpool.get(latest_tail_page_index).num_records == 512:
            new_page_number = latest_tail_number + 1
            num_column = self.table.num_columns + NUM_META_COLUMN
            ...
            self.table.num_tail_page += 1
            self.table.tail_directory.update({str(page_index): latest_tail_number+1})
            ...
            new_tail_number = latest_tail_number 
